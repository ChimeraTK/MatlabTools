...
bool mxIsRealScalar(const mxArray *a) {
  return mxIsNumeric(a) && !mxIsComplex(a) && (mxGetNumberOfElements(a) == 1);
}
bool mxIsPositiveRealScalar(const mxArray *a) {
  return mxIsNumeric(a) && !mxIsComplex(a) && (mxGetNumberOfElements(a) == 1) &&
         (mxGetScalar(a) > 0);
}
bool mxIsRealVector(const mxArray *a) {
  return mxIsNumeric(a) && !mxIsComplex(a) &&
         (mxGetNumberOfElements(a) == mxGetN(a));
}
bool mxIsPositiveRealVector(const mxArray *a) {
  return mxIsNumeric(a) && !mxIsComplex(a) &&
         (mxGetNumberOfElements(a) == mxGetN(a)) && (mxGetScalar(a) >= 0);
}
...
std::string getOrdinalNumerString(const unsigned int &n) {
  ...
  if (n > arrayOfOrdinalNumberStrings.size()) {
    ...
    return arrayOfOrdinalNumberStrings[n];
}

string mxArrayToStdString(const mxArray *pa) {
  char *ps = mxArrayToString(pa);
  string s = ps;
  mxFree(ps);
  return s;
}
...
typedef void (*CmdFnc)(unsigned int, mxArray **, unsigned int,
                       const mxArray **);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  try {
    ...
    if (it == vectorOfCommands.end()) {
      mexErrMsgTxt("Unknown command '" + cmd +
                   "'. Use mtca4u('help') to show some help information.");
      return;
    }
...
void getDMapFilePath(unsigned int, mxArray *plhs[], unsigned int,
                     const mxArray **) {
  plhs[0] =
      mxCreateString((BackendFactory::getInstance().getDMapFilePath()).c_str());
}
 ...
 *
 */
void setDMapFilePath(unsigned int, mxArray **, unsigned int nrhs,
                     const mxArray *prhs[]) {
  if (nrhs < 1)
    mexErrMsgTxt("Not enough input params.");
  if (nrhs > 1)
    mexWarnMsgTxt("Too many input arguments.");
 ...
 * @brief getDevice
 *
 * @param[in] dmapFileName File to be loaded or all in the current directory if
 * empty
 *
 */
...
void PrintHelp(unsigned int, mxArray **, unsigned int, const mxArray **) {
  ...
  mexPrintf("\n\nFor further help or bug reports please contact "
            "chimeratk-support@desy.de\n");
}
 ...
 *
 */
void openDevice(unsigned int, mxArray *plhs[], unsigned int nrhs,
                const mxArray *prhs[]) {
  if (nrhs < 1)
    mexErrMsgTxt("Not enough input arguments.");
  if (nrhs > 1)
 ...
 *
 */
void closeDevice(unsigned int, mxArray **, unsigned int nrhs,
                 const mxArray *prhs[]) {
  if (nrhs < 1)
    mexErrMsgTxt("Not enough input arguments.");
  if (nrhs > 1)
  ...
  // The backend factory will keep a copy, so a rebot backend for instance will
  // keep the device occupied if we just reset the device object. So we have to
  // really close it.
  if (openDevicesVector[deviceHandle]) { // check if device has been
                                         // closed/deleted already (avoid
                                         // accessing nullptr)
 ...
 *
 */
void getInfo(unsigned int nlhs, mxArray *plhs[], unsigned int nrhs,
             const mxArray **) {
  if (nrhs > 0)
    mexWarnMsgTxt("Too many input arguments.");
  if (nlhs > 1)
  ...
  plhs[0] = mxCreateStructArray(
      2, dims, (sizeof(field_names) / sizeof(*field_names)), field_names);

  unsigned int i = 0;

  for (DMapFilesParser::iterator it = parser.begin(); it != parser.end();
       ++it, ++i) {
    ...
    *mxGetPr(firmware_value) = 0;
    ...
    mxSetFieldByNumber(plhs[0], i, 0,
                       mxCreateString(it->first.deviceName.c_str()));
    ...
    mxSetFieldByNumber(plhs[0], i, 4,
                       mxCreateString(it->first.mapFileName.c_str()));
  }
}

std::string getFundamentalTypeString(
    ChimeraTK::RegisterInfo::FundamentalType fundamentalType) {
  if (fundamentalType == RegisterInfo::FundamentalType::numeric)
    return "numeric";
  if (fundamentalType == RegisterInfo::FundamentalType::string)
    return "string";
  if (fundamentalType == RegisterInfo::FundamentalType::boolean)
    return "boolean";
  if (fundamentalType == RegisterInfo::FundamentalType::nodata)
    return "nodata";
  if (fundamentalType == RegisterInfo::FundamentalType::undefined)
    return "undefined";
  return "unknown";
}
 ...
 *
 */
void getDeviceInfo(unsigned int nlhs, mxArray **plhs, unsigned int nrhs,
                   const mxArray *prhs[]) {
  if (nrhs < 1)
    mexErrMsgTxt("Not enough input arguments.");
  if (nrhs > 1)
  ...
  const char *field_names[] = {
      "name",        "nElementsPerChannel", "nChannels",
      "nDimensions", "fundamentalType",     "description"};

  plhs[0] = mxCreateStructMatrix(registerCatalogue.getNumberOfRegisters(), 1,
                                 (sizeof(field_names) / sizeof(*field_names)),
                                 field_names);

  unsigned int index = 0;
  for (RegisterInfoMap::const_iterator cit = registerCatalogue.begin();
       cit != registerCatalogue.end(); ++cit, ++index) {

    mxSetFieldByNumber(
        plhs[0], index, 0,
        mxCreateString(std::string(cit->getRegisterName()).c_str()));
    ...
    *mxGetPr(numDimensions) = cit->getNumberOfDimensions();
    ...
    mxSetFieldByNumber(
        plhs[0], index, 4,
        mxCreateString(getFundamentalTypeString(fundamentalType).c_str()));
 ...
 *
 */
void getRegisterInfo(unsigned int nlhs, mxArray *plhs[], unsigned int nrhs,
                     const mxArray *prhs[]) {
  if (nrhs < 3)
    mexErrMsgTxt("Not enough input arguments.");
  if (nrhs > 3)
  ...
  if (!mxIsChar(prhs[1]))
    mexErrMsgTxt("Invalid " + getOrdinalNumerString(2) + " input argument.");
  if (!mxIsChar(prhs[2]))
    mexErrMsgTxt("Invalid " + getOrdinalNumerString(3) + " input argument.");
  ...
  const char *field_names[] = {
      "name",        "nElementsPerChannel", "nChannels",
      "nDimensions", "fundamentalType",     "isIntegral",
      "isSigned",    "description"};
  plhs[0] = mxCreateStructMatrix(
      1, 1, (sizeof(field_names) / sizeof(*field_names)), field_names);

  mxSetFieldByNumber(
      plhs[0], 0, 0,
      mxCreateString(std::string(registerInfo->getRegisterName()).c_str()));
 ...
 *
 */
void getRegisterSize(unsigned int nlhs, mxArray *plhs[], unsigned int nrhs,
                     const mxArray *prhs[]) {
  if (nrhs < 3)
    mexErrMsgTxt("Not enough input arguments.");
  if (nrhs > 3)
  ...
  if (!mxIsChar(prhs[1]))
    mexErrMsgTxt("Invalid " + getOrdinalNumerString(1) + " input argument.");
  if (!mxIsChar(prhs[2]))
    mexErrMsgTxt("Invalid " + getOrdinalNumerString(2) + " input argument.");
 ...
 */
void readRegister(unsigned int nlhs, mxArray *plhs[], unsigned int nrhs,
                  const mxArray *prhs[]) {
  static const unsigned int pp_device = 0, pp_module = 1, pp_register = 2,
                            pp_offset = 3, pp_elements = 4;

  if (nrhs < 3)
    mexErrMsgTxt("Not enough input arguments.");
  if (nrhs > 5)
    mexWarnMsgTxt("To many input arguments.");
  if (nlhs > 1)
    mexErrMsgTxt("To many output arguments.");

  boost::shared_ptr<Device> device = getDevice(prhs[pp_device]);

  if (!mxIsChar(prhs[pp_module]))
    mexErrMsgTxt("Invalid " + getOrdinalNumerString(pp_module) +
                 " input argument.");
  if (!mxIsChar(prhs[pp_register]))
    mexErrMsgTxt("Invalid " + getOrdinalNumerString(pp_register) +
                 " input argument.");

  if ((nrhs > pp_offset) &&
      (!mxIsRealScalar(prhs[pp_offset]) || (mxGetScalar(prhs[pp_offset]) < 0)))
    mexErrMsgTxt("Invalid " + getOrdinalNumerString(pp_offset) +
                 " input argument.");

  if ((nrhs > pp_elements) && !mxIsPositiveRealScalar(prhs[pp_elements]))
    mexErrMsgTxt("Invalid " + getOrdinalNumerString(pp_elements) +
                 " input argument.");
  ...
  const uint32_t nElements =
      (nrhs > pp_elements) ? mxGetScalar(prhs[pp_elements]) : 0;
  ...
  auto registerContent =
      device->read<double>(moduleName / registerName, nElements, offset);

  // as both DeviceAccess and Matlab do their own memory allocation all we can
  // do is memcpy :-(
  plhs[0] = mxCreateDoubleMatrix(1, registerContent.size(), mxREAL);
  double *plhsValue = mxGetPr(plhs[0]);
  memcpy(plhsValue, registerContent.data(),
         registerContent.size() * sizeof(double));
}
 ...
 */
void writeRegister(unsigned int, mxArray **, unsigned int nrhs,
                   const mxArray *prhs[]) {
  static const unsigned int pp_device = 0, pp_module = 1, pp_register = 2,
  ...
  if (!mxIsNumeric(prhs[pp_value]) || mxIsComplex(prhs[pp_value]))
    mexErrMsgTxt("Invalid " + getOrdinalNumerString(pp_value) +
                 " input argument.");
  ...
  const uint32_t offset = (nrhs > pp_offset) ? mxGetScalar(prhs[pp_offset]) : 0;
  std::string registerPath = mxArrayToStdString(prhs[pp_module]) + '/' +
  ...
  if (arrayType == mxDOUBLE_CLASS) {
    writeToDevice<double>(device, registerPath, data, prhsValueElements,
                          offset);
  } else if (arrayType == mxUINT8_CLASS) {
    writeToDevice<uint8_t>(device, registerPath, data, prhsValueElements,
                           offset);
  } else if (arrayType == mxINT8_CLASS) {
    writeToDevice<int8_t>(device, registerPath, data, prhsValueElements,
                          offset);
  } else if (arrayType == mxINT16_CLASS) {
    writeToDevice<int16_t>(device, registerPath, data, prhsValueElements,
                           offset);
  } else if (arrayType == mxUINT16_CLASS) {
    writeToDevice<uint16_t>(device, registerPath, data, prhsValueElements,
                            offset);
  } else if (arrayType == mxINT32_CLASS) {
    writeToDevice<int32_t>(device, registerPath, data, prhsValueElements,
                           offset);
  } else if (arrayType == mxUINT32_CLASS) {
    writeToDevice<uint32_t>(device, registerPath, data, prhsValueElements,
                            offset);
  } else if (arrayType == mxINT64_CLASS) {
    writeToDevice<int64_t>(device, registerPath, data, prhsValueElements,
                           offset);
  } else if (arrayType == mxUINT64_CLASS) {
    writeToDevice<uint64_t>(device, registerPath, data, prhsValueElements,
                            offset);
  } else {
 ...
 *
 * Parameter: device, module, register, [offset], [elements], [mode], [singed],
 * [bit], [fracbit]
 */
void readDmaRaw(unsigned int nlhs, mxArray *plhs[], unsigned int nrhs,
                const mxArray *prhs[]) {
  static const unsigned int pp_device = 0, pp_module = 1, pp_register = 2,
                            pp_offset = 3, pp_elements = 4;
  static const unsigned int pp_mode = 5, pp_signed = 6, pp_bit = 7,
                            pp_fracbit = 8;
  ...
  if ((nrhs > pp_mode) && !mxIsPositiveRealScalar(prhs[pp_mode]))
    mexErrMsgTxt("Invalid " + getOrdinalNumerString(pp_mode) +
                 " input argument.");

  // FIXME: These don't make sense in raw mode. Shall we just skip the checks?
  // They are not used anyway.
  if ((nrhs > pp_signed) && !mxIsRealScalar(prhs[pp_signed]))
    mexErrMsgTxt("Invalid " + getOrdinalNumerString(pp_signed) +
                 " input argument.");
  if ((nrhs > pp_bit) && !mxIsPositiveRealScalar(prhs[pp_bit]))
    mexErrMsgTxt("Invalid " + getOrdinalNumerString(pp_bit) +
                 " input argument.");
  if ((nrhs > pp_fracbit) && !mxIsRealScalar(prhs[pp_fracbit]))
    mexErrMsgTxt("Invalid " + getOrdinalNumerString(pp_fracbit) +
                 " input argument.");
  ...
  // The mode can be 16 or 32 bit. This does not make sense because the real raw
  // stuff is 32 bits, it was there so we keep it in order not to break
  // compatibility.
  const uint32_t mode = (nrhs > pp_mode) ? mxGetScalar(prhs[pp_mode]) : 32;
  if ((mode != 32) && (mode != 16))
    mexErrMsgTxt("Invalid data mode.");

  // Number of 32 bit words to read through the "bus". This currently is the
  // only mode the raw accessor knows
  uint32_t nWords32Bit = nElements;
  if (mode == 16) {
    // The number of 32 bit words is only half the number of elements if the are
    // requested as 16 bit
    nWords32Bit /= 2;
  }

  // Notice: signedFlags, bits and fracBits have been removed as they were
  // not/cannot be used anyway
  ...
  auto accessor = device->getOneDRegisterAccessor<int32_t>(
      registerPath, nWords32Bit, offset, {AccessMode::raw});
  accessor.read();
 ...
 */
void readSequence(unsigned int nlhs, mxArray *plhs[], unsigned int nrhs,
                  const mxArray *prhs[]) {
  static const unsigned int pp_device = 0, pp_module = 1, pp_register = 2,
                            pp_channel = 3, pp_offset = 4, pp_elements = 5;
  ...
  if ((nrhs > pp_channel) && !mxIsPositiveRealVector(prhs[pp_channel]))
    mexErrMsgTxt("Invalid " + getOrdinalNumerString(pp_channel) +
                 " input argument.");

  if ((nlhs > 1) && !(nrhs > pp_channel))
    mexErrMsgTxt("Not enough input arguments.");
  if ((nlhs > 1) && (nrhs > pp_channel) &&
      (nlhs > mxGetNumberOfElements(prhs[pp_channel])))
    mexErrMsgTxt("Too many output arguments.");
  if ((nlhs > 1) && (nrhs > pp_channel) &&
      (nlhs < mxGetNumberOfElements(prhs[pp_channel])))
  ...
  auto twoDRegister = device->getTwoDRegisterAccessor<double>(
      mxArrayToStdString(prhs[pp_module]) + "/" +
      mxArrayToStdString(prhs[pp_register]));
  twoDRegister.read();

  const uint32_t offset = (nrhs > pp_offset) ? mxGetScalar(prhs[pp_offset]) : 0;
  const uint32_t elements = (nrhs > pp_elements)
                                ? mxGetScalar(prhs[pp_elements])
                                : (twoDRegister[0].size());
  ...
  unsigned int selectedChannels = (nrhs > pp_channel)
                                      ? mxGetNumberOfElements(prhs[pp_channel])
                                      : totalChannels;
  ...
  if (nlhs == selectedChannels) {
    for (unsigned int ic = 0; ic < nlhs; ic++) {
      ...
      unsigned int currentChannel =
          (nrhs > pp_channel) ? (mxGetPr(prhs[pp_channel])[ic] - 1) : ic;

      for (unsigned int is = 0; is < elements; is++) {
        unsigned int seqIndex = (is + offset);
        if (seqIndex >= size)
          mexErrMsgTxt("Data storage indexing went wrong!");
  ...
  // Store data in lhs matrix
    ...
    for (unsigned int ic = 0; ic < selectedChannels; ic++) {
      ...
      if (currentChannel < 0 || uint32_t(currentChannel) >= totalChannels)
        mexErrMsgTxt("Illegal Channel Index");
      ...
      for (unsigned int is = 0; is < elements; is++) {
        ...
        if (seqIndex >= size)
          mexErrMsgTxt("Data storage indexing went wrong! (1)");
        if (plhsIndex >= elements * selectedChannels)
          mexErrMsgTxt("Data storage indexing went wrong! (2)");
